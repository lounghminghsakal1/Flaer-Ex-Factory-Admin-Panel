...
export default function SerialInventoryFilters({
  draftFilters = {},
  setDraftFilters,
  appliedFilters = {},
  onApply,
    ...
    <div className="w-full mx-auto my-4">
      <div className="flex flex-wrap items-center gap-2 mb-3">
        ...
        <select
          ...
          onChange={(e) => setDraftFilters((prev) => ({ ...prev, by_status: e.target.value }))}
        >
          <option value="">All Statuses</option>
          {STATUS_OPTIONS.map((s) => (
            <option key={`status-${s.value}`} value={s.value}>{s.label}</option>
          ))}
        </select>
        ...
        <div className="flex items-center">
          <input
            ...
            className="border border-gray-300 text-xs px-2 h-8 w-44 rounded-l placeholder-gray-400 focus:outline-none focus:border-gray-500 transition text-gray-700"
            value={draftFilters.by_serial_number || ""}
            onChange={(e) =>
              setDraftFilters((prev) => ({ ...prev, by_serial_number: e.target.value }))
            }
            onKeyDown={(e) => { if (e.key === "Enter" && draftFilters.by_serial_number?.trim()) onApply(); }}
          />
          <span
            onClick={() => { if (draftFilters.by_serial_number?.trim()) onApply(); }}
            className="h-8 px-2 bg-primary flex items-center border border-gray-300 border-l-0 rounded-r cursor-pointer hover:scale-105 transition"
          >
            <SearchIcon size={13} color="white" />
          </span>

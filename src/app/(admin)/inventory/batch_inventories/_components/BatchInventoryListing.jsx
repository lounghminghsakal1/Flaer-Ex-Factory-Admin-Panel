...
export default function BatchInventoryListing({ batchData, currentPage, totalPages, setCurrentPage }) {
  const columns = [
    {
      key: "batch_code",
      label: "Batch Code",
    ...
    },
    {
      ...
      label: "SKU Code",
      render: (_, row) => (
        <span className="font-mono text-[11px] text-gray-600 bg-gray-100 py-0.5 rounded">
  ...
  return (
    <div>
      <DataTable
        ...
        getDetailsLink={(row) => `batch_inventories/${row.id}?`}
      />
    </div>
  );
}
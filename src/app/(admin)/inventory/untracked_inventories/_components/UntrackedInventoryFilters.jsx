...
function SkuNameDropdown({ value, onChange }) {
  ...
  const fetchSkus = async (query = "") => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/admin/api/v1/product_skus?${params}`);
          ...
          <div className="absolute z-50 mt-1 w-56 bg-white border border-gray-200 rounded shadow-md">
            ...
            <ul className="max-h-44 overflow-y-auto pb-1">
              {!search && (
                <li
                  className="text-xs px-3 py-1.5 hover:bg-gray-100 cursor-pointer text-gray-500 italic"
                  onClick={() => { onChange(""); setSelectedLabel(""); setOpen(false); setSearch(""); }}
                >
                  All SKU Names
                </li>
              )}
              {loading ? (
                ...
                options.map((opt) => (
                  <li
                    ...
                    onClick={() => { onChange(opt.id); setSelectedLabel(opt.sku_name); setOpen(false); setSearch(""); }}
                  >
                    {opt.sku_name}
                  </li>
                ))
...
const DUMMY_UNTRACKED_NUMBERS = [
  { id: "EXP-GRN10008-40",  untracked_number: "EXP-GRN10008-40"  },
  { id: "EXP-GRN10017-65",  untracked_number: "EXP-GRN10017-65"  },
  { id: "EXP-GRN10019-75",  untracked_number: "EXP-GRN10019-75"  },
  { id: "EXP-GRN10022-79",  untracked_number: "EXP-GRN10022-79"  },
  { id: "EXP-GRN10023-80",  untracked_number: "EXP-GRN10023-80"  },
  { id: "EXP-GRN10024-81",  untracked_number: "EXP-GRN10024-81"  },
];

export default function UntrackedInventoryFilters({
  draftFilters = {},
  setDraftFilters,
  appliedFilters = {},
  onApply,
  ...
  const fetchNodes = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/admin/api/v1/locations/nodes?only_names=true`);
      const json = await res.json();
      if (json.status === "failure") throw new Error(json?.errors?.[0] ?? "Failed");
  ...
  return (
    <div className="w-full mx-auto my-4">
      <div className="flex flex-wrap items-center gap-2 mb-3">
        ...
        {/* BY UNTRACKED NUMBER */}
        <SearchableDropdown
          options={DUMMY_UNTRACKED_NUMBERS}
          value={draftFilters.by_untracked_number}
          onChange={(val) => setDraftFilters((prev) => ({ ...prev, by_untracked_number: val }))}
          placeholder="All Untracked Nos."
          labelKey="untracked_number"
          valueKey="id"
          loading={false}
        />
        ...
        {skuFilterType === "by_product_sku" && (
          <SkuNameDropdown
            ...
            onChange={(id) => setDraftFilters((prev) => ({ ...prev, by_product_sku: id, by_sku_code: "" }))}
          />
        )}
        ...
        {skuFilterType === "by_sku_code" && (
          <div className="flex items-center">
            <input
              ...
              onKeyDown={(e) => { if (e.key === "Enter" && draftFilters.by_sku_code?.trim()) onApply(); }}
            />
            <span
